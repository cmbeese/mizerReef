...
#' Set sponge parameters for mizerReef
#'
...
#'      category. All sponge-related parameters are stored in the
#'      `sponge_params` slot of the params object.
#'
#'      The interaction strength (e.g. `interaction_encrusting`) for
#'      each species determines how strongly that group feeds on each
#'      sponge category. This can be set via the corresponding column
#'      in the species parameter data frame, or directly via the
#'      `UR_interaction` argument. If neither is provided, all
#'      interaction strengths are set to zero and a warning is issued.
#'
#' @param params A `MizerParams` object.
#' @param encrusting_initial Numeric vector. Initial abundance at size for encrusting sponges. Must be positive. Default is NULL.
#' @param massive_initial Numeric vector. Initial abundance at size for massive sponges. Must be positive. Default is NULL.
#' @param cryptic_initial Numeric vector. Initial abundance at size for cryptic sponges. Must be positive. Default is NULL.
#' @param branching_initial Numeric vector. Initial abundance at size for branching sponges. Must be positive. Default is NULL.
#' @param UR_interaction Optional. A named list or array with one or more resource interaction vectors (e.g. interaction_encrusting, interaction_massive, interaction_cryptic, interaction_branching), each of length equal to the number of species. If NULL, will use columns in species_params or set to zero. All values must be numeric and between 0 and 1.
#' @param encrusting_growth Numeric. Placeholder for encrusting sponge growth parameter. Default is NULL.
#' @param encrusting_scaling Numeric. Placeholder for encrusting sponge scaling parameter. Default is NULL.
#' @param massive_growth Numeric. Placeholder for massive sponge growth parameter. Default is NULL.
#' @param massive_scaling Numeric. Placeholder for massive sponge scaling parameter. Default is NULL.
#' @param cryptic_growth Numeric. Placeholder for cryptic sponge growth parameter. Default is NULL.
#' @param cryptic_scaling Numeric. Placeholder for cryptic sponge scaling parameter. Default is NULL.
#' @param branching_growth Numeric. Placeholder for branching sponge growth parameter. Default is NULL.
#' @param branching_scaling Numeric. Placeholder for branching sponge scaling parameter. Default is NULL.
#' @param sponge_colour Character string. Colour to use for sponge lines in plots. Default is "slateblue".
#' @details All sponge-related parameters (initial abundance, growth, scaling) are stored in the `sponge_params` slot of the params object. Resource interaction strengths are set in the species_params data frame. This function supports flexible multi-resource interaction via the UR_interaction argument.
#' @return A `MizerParams` object with updated sponge parameters (in `sponge_params` slot).
#' @concept sponge
#' @export
setSpongeParams <- function(params,
                            encrusting_initial = NULL,
                            massive_initial = NULL,
                            cryptic_initial = NULL,
                            branching_initial = NULL,
                            UR_interaction = NULL,
                            encrusting_growth = NULL,
                            encrusting_scaling = NULL,
                            massive_growth = NULL,
                            massive_scaling = NULL,
                            cryptic_growth = NULL,
    ...
    # Abundance checks
    for (cat in c("encrusting", "massive", "cryptic", "branching")) {
        vals <- get(paste0(cat, "_initial"))
        if (!is.null(vals)) {
            if (!is.numeric(vals)) stop(paste0(cat, "_initial must be numeric."))
            if (any(vals <= 0)) stop(paste0(cat, "_initial must be positive."))
            params@sponge_params[[paste0(cat, "_initial")]] <- vals
        }
    }

    # Interaction setup (array for all sponge morphologies)
    morphs <- c("encrusting", "massive", "cryptic", "branching")
    if (is.null(UR_interaction)) {
        for (m in morphs) {
            col <- paste0("interaction_", m)
            if (!(col %in% names(params@species_params))) {
                params@species_params[[col]] <- rep(0, no_sp)
                warning(paste0("You have not provided any values for ", col, ", so no species feed on ", m, " sponges."))
            }
        }
    } else {
        if (!is.list(UR_interaction) && !is.array(UR_interaction)) {
            stop("UR_interaction must be a named list or array with resource columns (e.g. interaction_encrusting, interaction_massive, interaction_cryptic, interaction_branching)")
        }
        for (m in morphs) {
            col <- paste0("interaction_", m)
            if (!is.null(UR_interaction[[col]])) {
                vals <- UR_interaction[[col]]
                if (length(vals) != no_sp) stop(paste0(col, " must have a value for every species."))
                if (!is.numeric(vals)) stop(paste0(col, " must be numeric."))
                if (any(vals < 0) || any(vals > 1)) stop(paste0(col, " values must be between 0 and 1."))
                params@species_params[[col]] <- vals
            } else if (!(col %in% names(params@species_params))) {
                params@species_params[[col]] <- rep(0, no_sp)
                warning(paste0("You have not provided any values for ", col, ", so no species feed on ", m, " sponges."))
            }
        }
    ...
    # Store growth and scaling params (placeholders)
    params@sponge_params$encrusting_growth <- encrusting_growth
    params@sponge_params$encrusting_scaling <- encrusting_scaling
    params@sponge_params$massive_growth <- massive_growth
    params@sponge_params$massive_scaling <- massive_scaling
    params@sponge_params$cryptic_growth <- cryptic_growth
    params@sponge_params$cryptic_scaling <- cryptic_scaling
    params@sponge_params$branching_growth <- branching_growth
    params@sponge_params$branching_scaling <- branching_scaling

    # Colour checks for sponge morphologies
    sponge_colours <- list(
        encrusting_colour = encrusting_colour,
        massive_colour = massive_colour,
        cryptic_colour = cryptic_colour,
        branching_colour = branching_colour
    )
    for (col_name in names(sponge_colours)) {
        col_val <- sponge_colours[[col_name]]
        if (!is.null(col_val)) {
            if (!is.character(col_val) || length(col_val) != 1 || is.na(grDevices::col2rgb(col_val)[1])) {
                stop(paste0(col_name, " must be a valid color name or hex code."))
            }
            params@linecolour[gsub("_colour", "", col_name)] <- col_val
        }
    }

    params@linetype["sponge"] <- "solid"
    params@time_modified <- lubridate::now()
    return(params)
}
