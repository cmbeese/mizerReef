...
[resources](#resources). All predator-prey interactions are
...
parameters were taken from [@Beese_2023] to allow for comparison
of results. 
...
Biomass estimates for most functional groups were based on data collected
from relatively un-fished reefs within a marine reserve in Bonaire 
[@Rogers_2014]. The survey methods used were unable to detect cryptic or 
nocturnal species, so estimates for carnivorous eels, nocturnal 
invertivores, and cryptobenthic predators were taken from the literature [@REFERENCES HERE]. Life history parameters for each functional group are 
derived from data on the chosen representative species, the most abundant 
in each group.

Fish less than 10 cm in length were excluded from analysis as their
abundance is not well captured by visual methods [@Ackerman_2000]. The
cutoff weight for observed biomass estimates was calculated for 
each functional group using length-weight relationships from FishBase.
The cutoff size is given in @tbl-biomass along with the observed biomass 
...
params@species_params %>%
    ...
    select(func_group,rep_species,biomass_observed, biomass_cutoff) %>%
    kable(col.names = c("Functional group","Species",
...
$$
R_j(w_p) = \frac{r_{peak}}{1 + e^{\left(\alpha (w - W_{refuge})\right)}}
$$ {#eq-refuge_simple}

Here $W_{refuge}$ defines the weight at which refuges transition from being 
widely available to becoming unavailable. 

$r_{peak}$ defines the maximum proportion of fish with access to predation 
...
$w$ has three sources: predation mortality $\mu_{p.i}(w)$, background
mortality $\mu_{ext.i}(w)$, fishing mortality $\mu_{f.i}(w)$. Background 
...
No fishing mortality was imposed in this iteration of the model, though 
the mizerReef package retains all functionality for simulating fishing 
scenarios from mizer.

<!-- ADD FISHING SCENARIOS HERE -->
...
groups and invertebrates feed on algae or detritus. 
