...
[@REFERENCES HERE]. Life history parameters for each functional group are 
...
the modeled reef and mirrors he way refuge was defined in @Rogers_2017 and 
@Rogers_2018. The **refuge density** describes the distribution of refuges 
...
mortality is composed of residual natural mortality $\mu_{nat.i}(w)$, and 
senescence $\mu_{sen.i}(w)$. The mortality rate for group $i$ is then given by: 
$$
...
We did not impose any fishing mortality in this model. Parameters were based
on data from a marine reserve where fishing levels were low. Any mortality
by subsistence fishers should be captured in external mortality.
...
grams. The steady state abundance of plankton at size 1g is 
$\kappa=`r kappa`[g/m^{-2}]$. The slope of the plankton spectrum is
$\lambda=`r lambda`$. 
