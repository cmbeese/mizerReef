...
To make a mizer reef model, we need some data. First, let's load our
data into the environment. To make a mizerReef model, you need the
following: + a species parameter data frame + interaction matrix +
detritus and algae interaction matrix + refuge profile method + refuge
profile data

The species parameters should contain the observed biomasses and cutoff
size for each of the ten functional groups. All other functional group
parameters will have defaults. The interaction matrix should list a
value between 0 and 1 for the proportion of time these groups will
encounter each other for each predator species by prey species
combination. Similarly, the unstructured resource interaction matrix and
should contain values between 0 and 1 for each group with the proportion
of time that they spend feeding on detritus and algae.
