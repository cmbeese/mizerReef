# Find the proportion of fish vulnerable to being encountered by predators at each time step
...
``` r
reefVulnerable(params, n, n_pp, n_other, t, new_rd = NULL, ...)
```
...
- new_rd:
  ...
  Optional numeric vector of refuge densities for each size bin, as
  returned by
  [`reefDegrade()`](https://cmbeese.github.io/mizerReef/reference/reefDegrade.md).
  If NULL (the default), the refuge density is taken from the current
  competitive method parameters (no degradation) or computed via
  [`reefDegrade()`](https://cmbeese.github.io/mizerReef/reference/reefDegrade.md)
