...
#> Warning: `kappa` is still at mizer's uncalibrated default (1e11) while
#> `biomass_observed` is set. `gamma` has been derived to match this placeholder
#> resource scale, so encounter with real fish/invert prey will be numerically
#> negligible until the model's scale is calibrated -- run
#> `calibrateReefBiomass()` before `matchReefGrowth()`/ `matchBiomasses()`, or
#> predators may converge on the right total biomass while eating ~0% fish prey.
```

These messages let us know when a default value was used for some
parameter you didn’t provide, and they come from two different places:

- **mizer’s own defaults** – search volume (`gamma`), maximum
  consumption rate (`h`), and the like. These are exactly what you’d see
  calling
  [`mizer::newMultispeciesParams()`](https://sizespectrum.org/mizer/reference/newMultispeciesParams.html)
  directly; see mizer’s [Setting
  Parameters](https://sizespectrum.org/mizer/reference/setParams.html)
  reference page for what each one is and how it’s calculated.
- **mizerReef’s own defaults** – `refuge_user`, `satiation`,
  `algae_growth_initial` and the rest of the reef-specific mechanism,
  listed in
  [`vignette("mizerReef")`](https://cmbeese.github.io/mizerReef/articles/mizerReef.md)’s
  \[Setting species parameters\] and \[Creating your first model\]
  sections and in
  [`vignette("tuning-diet-composition")`](https://cmbeese.github.io/mizerReef/articles/tuning-diet-composition.md).

Either way, you can change the value later if you have a better
estimate, using
[`species_params()`](https://sizespectrum.org/mizer/reference/species_params.html)
or one of the `set*()` functions.

With a full species table, this wall of messages gets long.
[`newReefParams()`](https://cmbeese.github.io/mizerReef/reference/newReefParams.md)
(and every other model-building function in mizerReef) takes an
`info_level` argument to control it: `info_level = 1` keeps only the
reports that say something went differently from what you actually asked
for (dropping the routine default-value fills), and `info_level = 0` is
silent. For example:
...
params <- params |>
    ...
    reefSteady() |> reefSteady() |> reefSteady()
#> ℹ Reached the convergence tolerance after 57 years. The biomasses change at up to 0.0035 per year.
#> ℹ Reached the convergence tolerance after 1.5 years. The biomasses change at up to 0.0029 per year.
#> ℹ Reached the convergence tolerance after 1.5 years. The biomasses change at up to 0.0024 per year.
#> ℹ Reached the convergence tolerance after 1.5 years. The biomasses change at up to 0.002 per year.
#> ℹ Reached the convergence tolerance after 1.5 years. The biomasses change at up to 0.0017 per year.
#> ℹ Reached the convergence tolerance after 1.5 years. The biomasses change at up to 0.0014 per year.
...
stock-recruitment relationship, from 0 (maximum compensation:
recruitment saturates quickly against spawning biomass) to 1 (mizer’s
default, no compensation: recruitment is directly proportional to
spawning biomass).

Reef models add several sources of non-linearity beyond a standard mizer
model – coupled algae/detritus/plankton dynamics, refuge mechanisms
whose effect on predation mortality is itself density-dependent, and the
iterative
[`calibrateReefBiomass()`](https://cmbeese.github.io/mizerReef/reference/calibrateReefBiomass.md)/[`matchReefGrowth()`](https://cmbeese.github.io/mizerReef/reference/matchReefGrowth.md)
loops used to reach them. With no reproductive compensation
(`reproduction_level = 1`), there’s nothing to damp these interacting
non-linearities, and
[`reefSteady()`](https://cmbeese.github.io/mizerReef/reference/reefSteady.md)’s
search for an equilibrium (where it repeatedly adjusts `erepro` to drive
births toward deaths) can fail to converge or oscillate. Moderate
compensation (`reproduction_level = 0.5`) provides just enough
stabilizing feedback to damp this without itself being steep enough to
cause the solver to overshoot, which is what happens as compensation
gets stronger still (`reproduction_level` near 0). This isn’t purely a
numerical convenience, either: many reef fish have an open recruitment
system, with larvae dispersing widely and local recruitment only weakly
tied to local spawning stock, which is itself closer to moderate than to
zero or maximum compensation.
...
rates. Afterwards, you can move it toward 1 if you believe recruitment
is very weakly density-dependent for your system, or toward 0 to test
recovery dynamics from overfishing – but moderate values are usually
both the most numerically stable and the most biologically defensible
default for reef fish with typical connectivity.
...
#> ℹ Reached the convergence tolerance after 33 years. The biomasses change at up to 0.018 per year. The component `algae` (up to 0.57 per year) is also changing. Components are not included in the biomass drift above, although this run does advance them; see `attr(getSteadyResidual(params), "other")` for the rates themselves.
#> ℹ Reached the convergence tolerance after 25.5 years. The biomasses change at up to 0.0015 per year. The component `algae` (up to 0.21 per year) is also changing. Components are not included in the biomass drift above, although this run does advance them; see `attr(getSteadyResidual(params), "other")` for the rates themselves.
#> ℹ Reached the convergence tolerance after 21 years. The biomasses change at up to 0.0032 per year. The component `algae` (up to 0.14 per year) is also changing. Components are not included in the biomass drift above, although this run does advance them; see `attr(getSteadyResidual(params), "other")` for the rates themselves.
#> ℹ Reached the convergence tolerance after 16.5 years. The biomasses change at up to 0.0013 per year.
#> Warning: ! For the following species `erepro` has been increased to the smallest possible value: erepro[pred_eng] = 0.00293; erepro[pred_grab] = 5.79; erepro[eels] = 0.000419
#> ! The following species require an unrealistic value greater than 1 for `erepro`: pred_plank, farm_damsel
...
internally, which balances each pool’s production against consumption
(see
[`vignette("model-description")`](https://cmbeese.github.io/mizerReef/articles/model-description.md)‘s
Algae and Detritus sections for exactly what’s held fixed for each).
That balance says nothing about the pools’ *absolute* scale, though, and
reaching it usually leaves algae and detritus biomass far from a
realistic standing stock: every biomass-correction step above
([`calibrateReefBiomass()`](https://cmbeese.github.io/mizerReef/reference/calibrateReefBiomass.md)
in particular) necessarily rescales the species’ interaction strength
with algae/detritus (`rho_algae`/ `rho_detritus`) to preserve feeding
level as fish abundances are corrected – the same principle mizer itself
uses for `gamma`/search volume – and that repeated rescaling drives the
*absolute* biomass scale down over the course of calibration, even
though nothing about it is wrong.

The fix is a final, deliberately non-disruptive correction:
[`rescale_algae()`](https://cmbeese.github.io/mizerReef/reference/rescale_algae.md)/[`rescale_detritus()`](https://cmbeese.github.io/mizerReef/reference/rescale_detritus.md)
multiply a pool’s biomass by a factor and divide the consuming species’
interaction strength by the same factor, so **total consumption – and
therefore the already-tuned fish sub-model – is completely unaffected**.
`detritus_lifetime() <-` is a convenience wrapper around
[`rescale_detritus()`](https://cmbeese.github.io/mizerReef/reference/rescale_detritus.md)
that targets a turnover time (biomass / consumption rate) instead of a
raw factor; there is no equivalent setter for algae, so target its
standing biomass directly with
...
because it doesn’t touch consumption, it doesn’t need to be redone after
a biomass or growth match, and running it before one would just be
undone by the next
[`calibrateReefBiomass()`](https://cmbeese.github.io/mizerReef/reference/calibrateReefBiomass.md)
call.
...
5.  **Final tuning**: Adjust reproduction parameters for realistic
    dynamics
6.  **Rescale resources**: Correct algae/detritus to a realistic
    absolute scale, last, since it doesn’t disturb anything tuned before
    it
...
- [`rescale_algae()`](https://cmbeese.github.io/mizerReef/reference/rescale_algae.md),
  `detritus_lifetime()<-`: Correct absolute resource scale without
  disturbing consumption
...
&nbsp;
    ...
    #> [46] Rcpp_1.1.2          systemfonts_1.3.2   xfun_0.61          
    #> [49] tibble_3.3.1        tidyselect_1.2.1    farver_2.1.2       
    #> [52] htmltools_0.5.9     labeling_0.4.3      rmarkdown_2.32     
